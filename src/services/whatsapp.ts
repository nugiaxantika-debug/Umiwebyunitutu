        ...
        if (ytDownload && ytDownload.status && ytDownload.download && ytDownload.download.url) {
          try {
            ...
            try {
              ...
              execSync(`"${ffmpegPath}" -y -i ${tmpRaw} -c:a libopus -b:a 48k -vbr on -compression_level 10 -frame_duration 20 -application voip ${tmpFixedAudio}`);
        ...
        if (ytDownload && ytDownload.status && ytDownload.download && ytDownload.download.url) {
          ...
          try {
            ...
            try {
              ...
              execSync(`"${ffmpegPath}" -y -i ${tmpRaw} -c:v libx264 -preset veryfast -crf 28 -c:a aac -b:a 128k ${tmpFixedMp4}`);
